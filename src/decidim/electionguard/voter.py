...
class ProcessCreateElection(VoterElectionStep):
    ...
    def process_message(self, message_type: str, message: dict):
        ...
        self.voter_state.election = ElectionDescription.from_json_object(
            complete_election_description(self.election_description)
        )
...
class ProcessJointElectionKey(VoterElectionStep):
    ...
    def process_message(self, message_type: str, message: dict):
        ...
        self.voter_state.election_metadata, self.voter_state.election_context = get_optional(
            self.voter_state.election_builder.build()
        )
